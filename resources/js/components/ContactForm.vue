...
<template>
    <div class="mb-6 sm:mb-8 bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow">
        <div class="flex justify-between items-center" :class="{ 'mb-4': isExpanded }">
            <h2 class="text-lg sm:text-xl font-semibold">{{ $t('phonebook.add_contact') }}</h2>
            <button
                @click="toggleForm"
        ...
        >
            <form v-show="isExpanded" @submit.prevent="addContact" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('phonebook.name') }}</label>
                    <input
                    ...
                    />
                    <p v-if="errors.name" class="mt-1 text-sm text-red-500">{{ $t('phonebook.error.name_required') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('phonebook.phone_number') }}</label>
                    <input
                    ...
                    />
                    <p v-if="errors.phone_number" class="mt-1 text-sm text-red-500">{{ $t('phonebook.error.phone_unique') }}</p>
                </div>
                <button
                    type="submit"
